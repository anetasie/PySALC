...
def projection_ci():
	#projection()
	conf()
	mode =	np.array( get_conf_results().parvals )
	low = np.array( get_conf_results().parmins )
	hi = np.array( get_conf_results().parmaxes )
	npars = len(hi)
...
def analyze_draws(draws,parnames, sigma=1, dict=False, verbose=True, means=False):
	...
	for i in range(npars):
		...
		out[i,0] = ui2._quantile(x, 0.5)
		if means:
			out[i,0] = np.mean(x)
		out[i,1] = ui2._quantile(x, (1-sigfrac)/2.0)
		out[i,2] = ui2._quantile(x, (1+sigfrac)/2.0)
	if verbose:
...
def proj_ci( sigma=1, verbose=True):
	get_conf().sigma = sigma
	conf()
	parnames = np.array(get_conf_results().parnames)
	npars = parnames.size
	fits = np.array(get_conf_results().parvals)
	lower = np.array(get_conf_results().parmins)
	upper = np.array(get_conf_results().parmaxes)
	out = np.zeros( (npars, 3) )
	out[:,0] = fits
	out[:,1] = fits + lower
	out[:,2] = fits + upper
...
def grid3by3( parnames, mins, maxes, dim, log=False, inv=False, defaultprior=True, priorshape=flat, originalscale=True, verbose=False, dataid=1):
	""" calc log posterior density over 3-d grid
		...
		if default prior is False, must specify priorshape and originalscale for eacher parameter
			"""
	...
	def eval_grid(paramspace, calc_stat, dataid):
		...
		for ii, parvals in enumerate(paramspace):
			...
			#	if np.sum(log)>0:
			#		if prior_inv:
			#			prior[log] = 1.0/parvals[log]
			#	if np.sum(inv)>0:
			#		if prior_inv:
			#			prior[inv] = 1.0/parvals[inv]
	...
	for ii, par in enumerate(parnames):
		...
		if np.sum(inv)>0:
			if inv[ii]==True:
				# create equally spaced on inverse scale, but store on original scale
